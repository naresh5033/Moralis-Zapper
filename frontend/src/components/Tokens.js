...
function Tokens({ wallet, chain, tokens, setTokens }) {
  async function getTokenBalances() {
    ...
    //sim to the NativeTkns comp, if we've res data, do some processing(price)
    //except we ve array this time, ve to loop thru that.
    if (response.data) {
  ...
  function tokenProcessing(t) {
    for (let i = 0; i < t.length; i++) { //convrt the tkn bal to decimals 1E18
      t[i].bal = (Number(t[i].balance) / Number(`1E${t[i].decimals}`)).toFixed(
        3
      ); 

      //now for the usd price
      t[i].val = (
  ...
  return (
    <>
      ...
      {tokens.length > 0 && ( //if we ve tkn then provide the table and map(data) then each
        <Table
          pageSize={6}
          noPagination={true}
          style={{ width: "900px" }}
